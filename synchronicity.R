...
sync.raster2.melt$cell <- factor(x = sync.raster2.melt$cell,
                         ...
                         ordered = TRUE)
#Rasterplot
AKT1_midbrain3_sync.raster.hc.plt <- ggplot(sync.raster2.melt, aes(time, cell))+
  geom_raster(aes(fill = `Ca2+`))+
  ...
  ggtitle("AKT1 midbrain 3 corr > 0.5")
...
AKT1_midbrain3_hiSUM.plt <- ggplot(hiSUM, aes(Time, Perc))+
  geom_line()+ 
  theme_pubr()


# GRID raster/sums
sync.plots <- align_plots(AKT1_midbrain3_sync.raster.hc.plt, hiSUM.plt, align = 'v', axis = 'l')
sync.AKT1midbrain3.grid <- plot_grid(sync.plots[[1]], AKT1_midbrain3_hiSUM.plt, ncol = 1, rel_heights = c(4.5,1))
