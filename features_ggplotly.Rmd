...
medians_df <- results_df %>%
  ...
  summarize(
    ...
    median_RFPtoNon_normalized = median(RFPtoNonRFP_normalized),
    .groups = 'drop'
  )
```
...
# RFP-to-NonRFP connections (normalized by no. of RFP cells)
```{r rfp-to_non-connections-normalized, warning=FALSE}
suppressMessages({
p11 <- ggplot(results_df, aes(Age, RFPtoNonRFP_normalized, color = Condition)) +
  geom_point(alpha = 0.3) +
  geom_line(data = medians_df, aes(Age, median_RFPtoNon_normalized, color = Condition), size = 1) +
  ggtitle("Proportions of RFP-nonRFP connections")
ggplotly(p11)
})


datatable(RFPtoNonRFP_normalized_all_p_values[which(P_value < 0.09)], options = list(
    pageLength = 10,
    autoWidth = TRUE,
    dom = 't'  # This option removes the search box and the pagination controls
  ),
...
# Percentage of redcells in big communities
```{r Redcells in big communities, warning=FALSE}
suppressMessages({
p12 <- ggplot(results_df, aes(Age, Percentage_in_Big_Communities, color = Condition)) +
  geom_point(alpha = 0.3) +
  geom_line(data = medians_df, aes(Age, median_RFPtoNon_normalized, color = Condition), size = 1) +
  ggtitle("Proportions of RFP-nonRFP connections")
ggplotly(p11)
