...
# Perm test for CTRLvsAKT1vsHRASV12 IRF8null clustcoeff ----
...
# Perm test for HRASV12 vs HRASV12_IRF8null Clustcoeff ----
...
# Perm test for CTRL vs CTRL_IRF8null Clustcoeff ----
...
# Perm test for CTRL vs AKT1 vs HRASV12 global_efficiency_rfp ----
# Function to calculate the median difference
median_diff <- function(x, y) {
  return(abs(median(x) - median(y)))
}
...
# Perm test for CTRL vs AKT1 vs HRASV12 IRF8 mutants global_efficiency_rfp ----
# Filter the dataframe to include only _IRF8null conditions
irf8null_conditions <- results_df %>% filter(grepl("_IRF8null", Condition))
...
# Perm test for IRF8+/+ vs IRF8-/- global_efficiency_rfp ----
perm_test_irf8_comparisons <- function(results_df, condition1, condition2) {
  # Unique ages in the dataset
  ages <- unique(results_df$Age)
...
# Perm test for CTRL vs AKT1 vs HRASV12 frequency ----
# Function to calculate the median difference
median_diff <- function(x, y) {
  return(abs(median(x) - median(y)))
}
...
# Perm test for CTRL vs AKT1 vs HRASV12 IRF8 mutants frequency ----
# Filter the dataframe to include only _IRF8null conditions
irf8null_conditions <- results_df %>% filter(grepl("_IRF8null", Condition))
...
# Perm test for CTRL vs AKT1 vs HRASV12 frequency_rfp ----
# Unique ages in the dataset
ages <- unique(results_df$Age)
...
# Perm test for CTRL vs AKT1 vs HRASV12 IRF8 mutants frequency_rfp ----
# Filter the dataframe to include only _IRF8null conditions
irf8null_conditions <- results_df %>% filter(grepl("_IRF8null", Condition))
...
# Perm test for IRF8+/+ vs IRF8-/- frequency_rfp ----
# Function to perform permutation test for AKT1 vs AKT1_IRF8null, HRASV12 vs HRASV12_IRF8null, CTRL vs CTRL_IRF8null
