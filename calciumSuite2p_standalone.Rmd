...
saveRDS(spks, file = "~/calcium-clustering/data/ID0151_spks.rds")
write.csv(spks, "~/calcium-clustering/data/ID0151_spks.csv")
...
saveRDS(spksthresholded, file = "~/calcium-clustering/data/ID0151_spksthresholded.rds")
write.csv(spksthresholded, "~/calcium-clustering/data/ID0151_spksthresholded.csv")
...
saveRDS(spksSUM, file = "~/calcium-clustering/data/ID0151_spksSUM.rds")
write.csv(spksSUM, file = "~/calcium-clustering/data/ID0151_spksSUM.csv")
...
saveRDS(frequency, file = "~/calcium-clustering/data/ID0151_mean_frequency.rds")
write.csv(frequency, file = "~/calcium-clustering/data/ID0151_mean_frequency.csv")
...
saveRDS(spksSUM2, file = "~/calcium-clustering/data/ID0151_spksSUM2.rds")
write.csv(spksSUM2, file = "~/calcium-clustering/data/ID0151_spksSUM2.csv")
...
raster.hc <- ggplot(meltPeaks, aes(time, cell))+
  ...
  # ggtitle("ID0151 hclust", subtitle = sprintf("Mean frequency is: %s events/min", round(frequency, digits = 3)))
...
# Save data
ggsave(plot = grid, file = "~/calcium-clustering/plots/ID0151.grid-allcells.png", 
       device = "png",  bg = "white",
       width = 20, height = 15, units = "cm", dpi = 320,
       scale = 2)
saveRDS(posXY, file = "~/calcium-clustering/data/ID0151_posXY.rds")
write.csv(posXY, file = "~/calcium-clustering/data/ID0151_posXY.csv")
...
saveRDS(RFPsum, file = "~/calcium-clustering/data/ID0151_RFPsum.rds")
write.csv(RFPsum, file = "~/calcium-clustering/data/ID0151_RFPsum.csv")
...
saveRDS(RFPsum2, file = "~/calcium-clustering/data/ID0151_RFPsum2.rds")
write.csv(RFPsum2, file = "~/calcium-clustering/data/ID0151_RFPsum2.csv")
...
RFP.raster <- ggplot(meltPeaks.RFP, aes(time, cell))+
  ...
  theme(legend.position = "none",
        ...
        plot.title = element_text(colour = "red", hjust = .5))+
  ggtitle("ID0151 RFP+", subtitle = sprintf("Mean frequency is: %s events/min", round(frequency.RFP, digits = 3)))
...
ID0151.clustcoeff.RFP <- transitivity(graph.RFP)

# Global efficiency RFP (NaN if there are no connected triples in the graph)
ID0151.globaleff.RFP <- global_efficiency(graph.RFP)
...
ID0151.pca.eigenvalues <- get_eigenvalue(pca)
...
ID0151.pca.RFP.eigenvalues <- get_eigenvalue(pca.RFP)
...
ggsave(plot = arranged, file = "~/calcium-clustering/plots/ID0151.whole.RFP.png", 
       device = "png",  bg = "white",
       width = 20, height = 15, units = "cm", dpi = 320,
       scale = 2)
...
# Clustering coefficient RFP
ID0151.clustcoeff <- transitivity(graph)

# Global efficiency RFP
ID0151.globaleff <- global_efficiency(graph)
...
# Save graph
ggsave(plot = graph.plt, file = "~/calcium-clustering/plots/ID0151.graph.png", 
       device = "png",  bg = "white",
       width = 20, height = 15, units = "cm", dpi = 320,
       scale = 2)
...
graph.btw.plt <- ggraph(graph, 
                                         ...
                                         scale_size_manual(values = c("0" = 5,
                                          ...
                                          ggtitle("ID0151 RFP+ highlighted")+
                                          scale_y_reverse() #this is because in images/movies y axis in coordinates is reversed

# Save graph
ggsave(plot = graph.btw.plt, file = "~/calcium-clustering/plots/ID0151.graph.btw.plt.png", 
       device = "png",  bg = "white",
       width = 20, height = 15, units = "cm", dpi = 320,
       scale = 2)
...
ID0151.psd.mean <- summarise(psd.melt, ID0151.mean = mean(PSD), .by = "frequency")
...
ID0151.psd.RFP.mean <- summarise(psd.RFP.melt, ID0151.RFP.mean = mean(PSD), .by = "frequency")
```
