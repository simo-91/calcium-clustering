...
ID0034.posXY <- data.frame()
for (i in 1:length(stat)) {
  ID0034.posXY <- rbind(ID0034.posXY, stat[[i]][["med"]])
}

ID0034.posXY$Cell <- as.numeric(0:(nrow(ID0034.posXY)-1))
colnames(ID0034.posXY) <- c('Y','X','Cell')
ID0034.posXY
```
...
ID0034.posXY$Positive <- iscell$V1
ID0034.posXY <- subset(ID0034.posXY, Positive == 1, select = c(Y,X,Cell))
positives <- ID0034.posXY$Cell
...
saveRDS(spks, file = "ID0034_spks.rds")
write.csv(spks, "ID0034_spks.csv")
```
...
saveRDS(spksthresholded, file = "ID0034_spksthresholded.rds")
write.csv(spksthresholded, "ID0034_spksthresholded.csv")
```
...
saveRDS(spksSUM, file = "ID0034_spksSUM.rds")
write.csv(spksSUM, file = "ID0034_spksSUM.csv")
ID0034.spksSUM.plt <- ggplot(spksSUM, aes(Time, Perc))+
  geom_line()+ 
  theme_pubr()
ID0034.spksSUM.plt
```
...
saveRDS(spksSUM2, file = "ID0034_spksSUM2.rds")
write.csv(spksSUM2, file = "ID0034_spksSUM2.csv")
ID0034.spksSUM2.plt <- ggplot(spksSUM2, aes(Time, Mean))+
  geom_line()+ 
  theme_pubr()+
  geom_smooth()+
  ylab("Ca2+")+
  ylim(0, NA)
ID0034.spksSUM2.ylim <- layer_scales(ID0034.spksSUM2.plt)$y$get_limits()
ID0034.spksSUM2.plt
```
```{r average activity/cell}
# Average activity PER CELL (deconvolved peaks) ---------------------------
ID0034.posXY$Mean <- rowMeans(spks)
ID0034.posXY
```
...
ID0034.meltPeaks <- melt(dfpeaks, id = "time")
colnames(ID0034.meltPeaks) <- c('time','cell','Ca2+')
...
ID0034.meltPeaks$cell <- factor(x = ID0034.meltPeaks$cell,
                         levels = peaks.rows$peaks.rows[peaks.order], 
                         ordered = TRUE)

# Ggplot raster with dendro order
ID0034.raster.hc <- ggplot(ID0034.meltPeaks, aes(time, cell))+
  geom_raster(aes(fill = `Ca2+`))+
  # geom_line(aes(color = RFP), alpha = .2)+
  # scale_y_discrete(breaks = levels(ID0034.meltPeaks$RFP))+
...
# GRID raster/sums
plots <- align_plots(ID0034.raster.hc, ID0034.spksSUM.plt, align = 'v', axis = 'l')
ID0034.grid <- plot_grid(plots[[1]], ID0034.spksSUM.plt, ncol = 1, rel_heights = c(4.5,1))
ID0034.grid
```
...
```{r HRAS/AKT1 cells}
########################################### RFP ####################################
ID0034.RFPcells <- unique(scan("RFPcells.R", sep = ","))

# Calculating percentage of active RFP cells over time (using deconvolved peaks)--------------------
RFPt <- subset(spksthresholded, rownames(spksthresholded) %in% ID0034.RFPcells) #select only RFP cells
...
saveRDS(RFPsum, file = "ID0034_RFPsum.rds")
write.csv(RFPsum, file = "ID0034_RFPsum.csv")

# Plot
ID0034.RFPsum.plt <- ggplot(RFPsum, aes(Time, Perc))+
  geom_line()+
  theme_pubr()
ID0034.RFPsum.plt
```
...
RFP <- subset(spks, rownames(spks) %in% ID0034.RFPcells) #select only RFP cells
RFPsum2 <- as.data.frame(colSums(RFP)/nrow(RFP)) #normalized by number of RFP cells
RFPsum2$Time <- 0:(nrow(RFPsum2)-1)
saveRDS(RFPsum2, file = "ID0034_RFPsum2.rds")
write.csv(RFPsum2, file = "ID0034_RFPsum2.csv")

ID0034.RFPsum2.plt <- ggplot(RFPsum2, aes(Time, `colSums(RFP)/nrow(RFP)`))+
  geom_line()+
  theme_pubr()+
  geom_smooth()+
  ylab("Deconvolved activity")+
  ylim(0, NA)
ID0034.RFPsum2.plt.ylim <- layer_scales(ID0034.RFPsum2.plt)$y$get_limits()
ID0034.RFPsum2.plt
```
...
ID0034.meltPeaks.RFP <- melt(dfpeaks.RFP, id = "time")
colnames(ID0034.meltPeaks.RFP) <- c('time','cell','Ca2+')
...
ID0034.meltPeaks.RFP$cell <- factor(x = ID0034.meltPeaks.RFP$cell,
                         levels = RFP.rows$RFP.rows[RFP.order], 
                         ordered = TRUE)

# Ggplot rasterwith dendro order
ID0034.RFP.raster <- ggplot(ID0034.meltPeaks.RFP, aes(time, cell))+
  geom_raster(aes(fill = `Ca2+`))+
...
# GRID raster/sums
plots <- align_plots(ID0034.RFP.raster, ID0034.RFPsum.plt, align = 'v', axis = 'l')
ID0034.RFP.grid <- plot_grid(plots[[1]], ID0034.RFPsum.plt, ncol = 1, rel_heights = c(3.5,1))
ID0034.RFP.grid
```

```{r Isolate HRAS coordinates from posXY dataframe}
# Isolate RFP+ XY from posXY dataframe ------------------------------------
ID0034.posXY$RFP <- ID0034.posXY$Cell %in% ID0034.RFPcells

ID0034.posXY.RFP <- ID0034.posXY[which(ID0034.posXY$RFP=="TRUE"),]

saveRDS(ID0034.posXY.RFP, file = "ID0034.posXY.RFP.rds")
write.csv(ID0034.posXY.RFP, file = "ID0034.posXY.RFP.csv")
ID0034.posXY.RFP
```
...
ID0034.posXY$Mean.dF <- rowMeans(dFPOS)
ID0034.posXY$Mean.dF.N <- apply(as.matrix(ID0034.posXY$Mean.dF), 2,
...
# dF.RFP <- subset(spksthresholded, rownames(dFPOS) %in% ID0034.RFPcells) #select only RFP cells
# ##ggplot to show percentage of RPF+ cells over time
# RFPsum <- as.data.frame(colSums(RFP))
# RFPsum$Time <- 0:(nrow(RFPsum)-1)
# RFPsum$Perc <- RFPsum$`colSums(RFP)`/nrow(RFP)*100
# ID0034.RFPsum.dF.plt <- ggplot(RFPsum, aes(Time, Perc))+
#   geom_line()+
#   theme_pubr()
# ID0034.RFPsum.dF.plt
# ```
...
# #plot
# ID0034.POS.aveF <- ggplot(dFx, aes(Time, `colSums(dFPOS)/nrow(dFPOS)`))+
#   geom_line()+
#   theme_pubr()+
#   ylab("Average dF/F")+
...
# #plot
# ID0034.RFP.aveF <- ggplot(FrawRFPx, aes(Time, `colSums(FrawRFP)/nrow(FrawRFP)`))+
#                               geom_line()+
#                               theme_pubr()+
...
# cut3 <- cutree(hc, k = 3)
# 
# ID0034.posXY$Cluster <- cut3
# 
# ggplot(ID0034.posXY, aes(X, Y, color = as.factor(Cluster), shape = as.factor(Cluster)))+
#   geom_point(size = 2)+
