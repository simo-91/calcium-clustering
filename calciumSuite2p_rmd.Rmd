...
ID0032.posXY$redcell <- redcell[,1]
```
...
ID0032.posXY <- subset(ID0032.posXY, Positive == 1, select = c(Y,X,Cell,redcell))
...
```{r frequency of calcium events/min for each cell}
# cell thresholded events divided by 60 sec
ID0032.posXY$frequency <- rowSums(spksthresholded)/60
ID0032.frequency <- mean(rowSums(spksthresholded)/60)
```
...
ID0032.meltPeaks <- melt(dfpeaks, id = "time")
colnames(ID0032.meltPeaks) <- c('time','cell','Ca2+')
...
ID0032.meltPeaks$cell <- factor(x = ID0032.meltPeaks$cell,
                         levels = peaks.rows$peaks.rows[peaks.order], 
                         ordered = TRUE)

# Ggplot raster with dendro order
ID0032.raster.hc <- ggplot(ID0032.meltPeaks, aes(time, cell))+
  geom_raster(aes(fill = `Ca2+`))+
  # geom_line(aes(color = RFP), alpha = .2)+
  # scale_y_discrete(breaks = levels(ID0032.meltPeaks$RFP))+
...
ID0032.meltPeaks.RFP <- melt(dfpeaks.RFP, id = "time")
colnames(ID0032.meltPeaks.RFP) <- c('time','cell','Ca2+')
...
ID0032.meltPeaks.RFP$cell <- factor(x = ID0032.meltPeaks.RFP$cell,
                         levels = RFP.rows$RFP.rows[RFP.order], 
                         ordered = TRUE)

# Ggplot rasterwith dendro order
ID0032.RFP.raster <- ggplot(ID0032.meltPeaks.RFP, aes(time, cell))+
  geom_raster(aes(fill = `Ca2+`))+
