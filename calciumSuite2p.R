...
ID0009.posXY <- data.frame()
for (i in 1:length(stat)) {
  ID0009.posXY <- rbind(ID0009.posXY, stat[[i]][["med"]])
}

ID0009.posXY$Cell <- as.numeric(0:(nrow(ID0009.posXY)-1))
colnames(ID0009.posXY) <- c('Y','X','Cell')
...
ID0009.posXY$Positive <- iscell$V1
ID0009.posXY <- subset(ID0009.posXY, Positive == 1, select = c(Y,X,Cell))
positives <- ID0009.posXY$Cell
...
saveRDS(spks, file = "ID0009_spks.rds")
write.csv(spks, "ID0009_spks.csv")
...
saveRDS(spksthresholded, file = "ID0009_spksthresholded.rds")
write.csv(spksthresholded, "ID0009_spksthresholded.csv")
...
saveRDS(spksSUM, file = "ID0009_spksSUM.rds")
write.csv(spksSUM, file = "ID0009_spksSUM.csv")
ID0009.spksSUM.plt <- ggplot(spksSUM, aes(Time, Perc))+
  geom_line()+ 
  theme_pubr()
...
saveRDS(spksSUM2, file = "ID0009_spksSUM2.rds")
write.csv(spksSUM2, file = "ID0009_spksSUM2.csv")
...
ID0009.posXY$Mean <- rowMeans(spks)
...
ID0009.raster.hc <- ggplot(meltPeaks, aes(time, cell))+
...
# GRID raster/sums
plots <- align_plots(ID0009.raster.hc, ID0009.spksSUM.plt, align = 'v', axis = 'l')
ID0009.grid <- plot_grid(plots[[1]], ID0009.spksSUM.plt, ncol = 1, rel_heights = c(4.5,1))


########################################### RFP ####################################
ID0009.RFPcells <- scan("RFPcells.R", sep = ",")

# Calculating percentage of active RFP cells over time --------------------
RFPt <- subset(spksthresholded, rownames(spksthresholded) %in% ID0009.RFPcells) #select only RFP cells
...
saveRDS(RFPsum, file = "ID0009_RFPsum.rds")
write.csv(RFPsum, file = "ID0009_RFPsum.csv")

# Plot
ID0009.RFPsum.plt <- ggplot(RFPsum, aes(Time, Perc))+
  geom_line()+
  theme_pubr()
...
RFP <- subset(spks, rownames(spks) %in% ID0009.RFPcells) #select only RFP cells
RFPsum2 <- as.data.frame(colSums(RFP)/nrow(RFP)) #normalized by number of RFP cells
RFPsum2$Time <- 0:(nrow(RFPsum2)-1)
saveRDS(RFPsum2, file = "ID0009_RFPsum2.rds")
write.csv(RFPsum2, file = "ID0009_RFPsum2.csv")

ID0009.RFPsum2.plt <- ggplot(RFPsum2, aes(Time, `colSums(RFP)/nrow(RFP)`))+
  geom_line()+
  theme_pubr()+
  geom_smooth()+
  ylab("Deconvolved activity")+
  ylim(0, NA)
ID0009.RFPsum2.plt.ylim <- layer_scales(ID0009.RFPsum2.plt)$y$get_limits()
...
ID0009.RFP.raster <- ggplot(meltPeaks.RFP, aes(time, cell))+
  geom_raster(aes(fill = `Ca2+`))+
  ...
  theme(legend.position = "none",
        ...
        axis.text.x = element_blank(),
        # axis.ticks.y = element_blank(),
        # axis.text.y = element_text(face=ID0009.posXY.RFP$Cell[which(ID0009.posXY.RFP$Member=="TRUE"),], "bold"),
        plot.title = element_text(colour = "red", hjust = .5))
  # ggtitle("ID0009 RFP+")

# GRID raster/sums
plots <- align_plots(ID0009.RFP.raster, ID0009.RFPsum.plt, align = 'v', axis = 'l')
ID0009.RFP.grid <- plot_grid(plots[[1]], ID0009.RFPsum.plt, ncol = 1, rel_heights = c(3.5,1))
...
ID0009.posXY$Mean.dF <- rowMeans(dFPOS)
ID0009.posXY$Mean.dF.N <- apply(as.matrix(ID0009.posXY$Mean.dF), 2,
                                function(x) (x - min(x))/(max(x)-min(x)))



# Calculating percentage of active RFP+ cells over time --------------------
dF.RFP <- subset(spksthresholded, rownames(dFPOS) %in% ID0009.RFPcells) #select only RFP cells
##ggplot to show percentage of RPF+ cells over time
RFPsum <- as.data.frame(colSums(RFP))
RFPsum$Time <- 0:(nrow(RFPsum)-1)
RFPsum$Perc <- RFPsum$`colSums(RFP)`/nrow(RFP)*100
ID0009.RFPsum.dF.plt <- ggplot(RFPsum, aes(Time, Perc))+
  geom_line()+
  theme_pubr()
...
ID0009.posXY$RFP <- ID0009.posXY$Cell %in% ID0009.RFPcells

ID0009.posXY.RFP <- ID0009.posXY[which(ID0009.posXY$RFP=="TRUE"),]

saveRDS(ID0009.posXY.RFP, file = "ID0009.posXY.RFP.rds")
write.csv(ID0009.posXY.RFP, file = "ID0009.posXY.RFP.csv")

#Averaging per timepoint (all cells)
# dFx <- as.data.frame(colSums(dFPOS)/nrow(dFPOS))
# dFx$Time <- 0:(nrow(dFx)-1)
# #plot
# ID0009.POS.aveF <- ggplot(dFx, aes(Time, `colSums(dFPOS)/nrow(dFPOS)`))+
#   geom_line()+
#   theme_pubr()+
#   ylab("Average dF/F")+
...
# #plot
# ID0009.RFP.aveF <- ggplot(FrawRFPx, aes(Time, `colSums(FrawRFP)/nrow(FrawRFP)`))+
#                               geom_line()+
#                               theme_pubr()+
...
# cut3 <- cutree(hc, k = 3)
# 
# ID0009.posXY$Cluster <- cut3
# 
# ggplot(ID0009.posXY, aes(X, Y, color = as.factor(Cluster), shape = as.factor(Cluster)))+
#   geom_point(size = 2)+
