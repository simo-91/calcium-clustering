```{r RFPcells for activity/hc/raster/dendrogram using thresholded data}
...
ID0034.RFPt.raster <- ggplot(meltPeaks.RFPt, aes(time, cell))+
  ...
  theme(legend.position = "none",
        ...
        # axis.text.y = element_text(face=ID0034.posXY.RFP$Cell[which(ID0034.posXY.RFP$Member=="TRUE"),], "bold"),
        plot.title = element_text(colour = "red", hjust = .5))
# ggtitle("ID0034 RFP+")

# GRID raster/sums
plots <- align_plots(ID0034.RFPt.raster, ID0034.RFPsum.plt, align = 'v', axis = 'l')
ID0034.RFPt.grid <- plot_grid(plots[[1]], ID0034.RFPsum.plt, ncol = 1, rel_heights = c(3.5,1))
ID0034.RFPt.grid
```